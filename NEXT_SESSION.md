...
- Practice prompt uses six stable string lanes: the target string is highlighted and the target note appears on that lane without fret numbers, fret markers, or answer-location reveal.
...
- Tests currently pass: 75 unit tests plus the Electron E2E workflow.
...
- Do not show fretboard answer locations, fret numbers, or note-position maps during active Practice.
...
Exact next task:
Have Robert run one real mic Daily Workout and validate the Practice Ready flow plus the string-lane prompt:
1. Run `npm run dev`.
2. Confirm the app opens without a running timer or mic.
3. Press `Start Practice`.
4. Confirm every Daily Workout prompt has a highlighted string lane and a note badge, without showing a fretboard answer location.
5. End the session.
